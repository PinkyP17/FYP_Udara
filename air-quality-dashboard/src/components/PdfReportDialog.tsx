...
const PdfReportDialog = () => {
    ...
    while (currentDate <= end && recordCount < maxRecords) {
      ...
      if (aggregation === "daily") {
        ...
        currentDate.setMinutes(
          currentDate.getMinutes() + Math.floor(Math.random() * 30) + 15
        );
      }
    }
  ...
  const calculateSummaryStats = (data, pollutants) => {
    ...
    pollutants.forEach((pollutantId) => {
      const values = data
        .map((d) => parseFloat(d[pollutantId]))
        .filter((v) => !isNaN(v));
      if (values.length > 0) {
        stats[pollutantId] = {
  ...
  const generatePDFReport = async (reportData, summaryStats) => {
    try {
      ...
      doc.setFont("helvetica", "normal");
      doc.text(
        "University Malaya Air Quality Monitoring System",
        pageWidth / 2,
        yPosition,
        { align: "center" }
      );
      ...
      const reportDetails = [
        ...
        `Aggregation: ${
          dataAggregation.charAt(0).toUpperCase() + dataAggregation.slice(1)
        }`,
